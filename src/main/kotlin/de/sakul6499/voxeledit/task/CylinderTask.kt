...
class CylinderTask(private val worldUID: UUID, midPoint: Vector, radius: Int, up: Int, down: Int, materials: Array<Triple<Material, Byte, Boolean>>, hollow: Boolean = false) : Task {
        ...
        for (y in yBegin..yEnd) {
            for (x in xBegin..xEnd) {
                for (z in zBegin..zEnd) {
                    ...
                    if (hollow) {
                        if (distance == radius) queue.add(Bundle(currentPosition, materials[random.nextInt(materials.size)]))
                    } else {
                        if (distance <= radius) queue.add(Bundle(currentPosition, materials[random.nextInt(materials.size)]))
                    }
                }
